...
void queryExamOrdersByDoctor(HIS_System* sys, const char* doctorId) {
	...
	// ---- 第一遍：超时检测 ----
	// 日期早于今天且状态为 Pending 的检查单，自动标记为超时封停
	ExamOrder* order = sys->examOrderHead;
	while (order != NULL) {
		if (strcmp(order->doctorId, doctorId) == 0) {
			if (isBeforeToday(order->date)
	...
	if (!found) {
		printf(">>> 今日暂无该医生开具的检查单。\n");
		pressEnterToContinue();
		return;
	}

	// 仅对今日待完成的检查单提供自动生成入口（仅 TEST_SYSTEM_DEBUG 模式下可用）
	tryAutoGenerateDoctorOrders(sys, doctorId, today);
	pressEnterToContinue();
}
